...
The goal with this project was to build a simple application (Ken's Sensor DataViewer) created by Visual Studio 2022 C# which reads continuously the sensor's temperature and humitidy values, which are
obtained from Raspberry PI 5 device with a help of DHT22 sensor trough network protocol. 
...
Visual Studio C# application show the sensor data in real time with help of two graphs. One for temperature and the ohter one for humitidy. The operating system of the device is Debian GNU/Linux 13 (trixie).
